...
export function search(mainEl: HTMLElement): void {
  const template = `
<div class="form-row">
  <h3>Name</h3>
...
<a href="#/products" class="btn-link">Search</a>
  ...
  const nameEl = mainEl.querySelector<HTMLInputElement>('input[name="name"]');
  ...
  const fmRadioEls = mainEl.querySelectorAll<HTMLInputElement>('input[name="fmRadio"]');
  ...
  const availabilityEls = mainEl.querySelectorAll<HTMLInputElement>('input[name="availability"]');
